...
public static class CodeGenHelpers
{
	...
	public static FieldAttributes ToFieldAttributes(this Visibility visibility)
	{
		return visibility switch
		{
			...
			Visibility.ProtectedInternal => FieldAttributes.FamORAssem,
			Visibility.Internal          => FieldAttributes.Assembly,
			Visibility.Private           => FieldAttributes.Private,
			var _                        => throw new NotImplementedException($"The specified visibility ({visibility}) is not supported.")
		};
	}
...
#pragma warning restore SYSLIB0050
	...
	public static MethodKind ToMethodKind(this EventKind kind)
	{
		return kind switch
		{
			EventKind.Static   => MethodKind.Static,
			EventKind.Normal   => MethodKind.Normal,
			EventKind.Virtual  => MethodKind.Virtual,
			EventKind.Abstract => MethodKind.Abstract,
			EventKind.Override => MethodKind.Override,
			var _              => throw new NotImplementedException($"The event kind ({kind}) is not supported.")
		};
	}
	...
	public static EventKind ToEventKind(this EventInfo @event)
	{
		...
		Debug.Assert(@event.AddMethod != null, "@event.AddMethod != null");
		Debug.Assert(@event.RemoveMethod != null, "@event.RemoveMethod != null");
	...
	/// <exception cref="ArgumentException">The method is not an add/remove accessor method.</exception>
	public static EventKind ToEventKind(this MethodInfo accessor)
	{
		// ensure the method is an add/remove accessor method
		if (!accessor.IsSpecialName || (!accessor.Name.StartsWith("add_", StringComparison.Ordinal) && !accessor.Name.StartsWith("remove_", StringComparison.Ordinal)))
	...
	public static PropertyKind ToPropertyKind(this MethodInfo accessor)
	{
		...
		if (!accessor.IsSpecialName || (!accessor.Name.StartsWith("get_", StringComparison.Ordinal) && !accessor.Name.StartsWith("set_", StringComparison.Ordinal)))
	...
	public static MethodAttributes ToMethodAttributes(this Visibility visibility)
	{
		return visibility switch
		{
			...
			Visibility.ProtectedInternal => MethodAttributes.FamORAssem,
	...
	public static MethodKind ToAccessorMethodKind(this PropertyKind kind)
	{
		return kind switch
		{
			PropertyKind.Static   => MethodKind.Static,
			PropertyKind.Normal   => MethodKind.Normal,
			PropertyKind.Virtual  => MethodKind.Virtual,
			PropertyKind.Abstract => MethodKind.Abstract,
			PropertyKind.Override => MethodKind.Override,
			var _                 => throw new NotImplementedException($"The property kind ({kind}) is not supported.")
		};
	}
	...
	public static MethodAttributes ToMethodAttributes(this MethodKind kind)
	{
		return kind switch
		{
			...
			MethodKind.Normal   => 0,
			MethodKind.Abstract => MethodAttributes.Virtual | MethodAttributes.NewSlot | MethodAttributes.Abstract,
			MethodKind.Virtual  => MethodAttributes.Virtual | MethodAttributes.NewSlot,
			MethodKind.Override => MethodAttributes.Virtual | MethodAttributes.ReuseSlot,
			var _               => throw new NotImplementedException($"The method kind ({kind}) is not supported.")
		};
	}
	...
	public static void EmitLoadConstant(ILGenerator msilGenerator, int x)
	{
		switch (x)
		{
			...
			default:
				if (x is >= sbyte.MinValue and <= sbyte.MaxValue) msilGenerator.Emit(OpCodes.Ldc_I4_S, (sbyte)x);
	...
	public static void EmitLoadConstant(ILGenerator msilGenerator, long x)
	{
		switch (x)
		{
			...
			default:
				if (x is >= sbyte.MinValue and <= sbyte.MaxValue)
				{
					msilGenerator.Emit(OpCodes.Ldc_I4_S, (sbyte)x);
				}
				else if (x is >= int.MinValue and <= int.MaxValue)
				{
