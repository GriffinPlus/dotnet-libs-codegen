...
class GeneratedField<T> : Member, IGeneratedField<T>, IGeneratedFieldInternal
{
	...
	/// <param name="isStatic">
	/// <c>true</c> to create a static field;<br/>
	/// <c>false</c> to create an instance field.
	/// </param>
		...
		Visibility visibility)
	{
		...
		var flags = Visibility.ToFieldAttributes();
	...
	void IGeneratedFieldInternal.ImplementFieldInitialization(Type createdType, ILGenerator msilGenerator)
	{
		...
		else if (mTypedProvideInitialValueCallback != null || mUntypedProvideInitialValueCallback != null)
		{
			...
			MethodInfo type_getTypeFromHandle = typeof(Type).GetMethod(nameof(Type.GetTypeFromHandle), [typeof(RuntimeTypeHandle)]);
			...
			else
			{
				...
				MethodInfo func_invoke = typeof(ProvideValueCallback).GetMethod("Invoke");
				Debug.Assert(func_invoke != null, nameof(func_invoke) + " != null");
				msilGenerator.Emit(OpCodes.Call, func_invoke);
